...
namespace apex_dma_radar
{
    internal static class Memory
    {
        ...
        {
            get => _process.GetModuleBase("r5apex_dx12.exe") != 0;
        }
        #endregion
        ...
        private static bool GetModuleBase()
        {
            try
            {
                ...
                _apexBase = _process.GetModuleBase("r5apex_dx12.exe");

                if (_apexBase == 0)
                    throw new DMAException("Unable to obtain Base Module Address. Game may not be running");
                else
                {
                    Program.Log($"Found r5apex_dx12.exe at 0x{_apexBase.ToString("x")}");
                    return true;
                }
            }
            catch (DMAShutdown) { throw; }
